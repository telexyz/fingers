...
fn getCursorPosition() TerminalError![2]u16 {
    var ret = c.write(c.STDOUT_FILENO, "\x1b[6n", 4);
    if (ret != 4)
    ...
    ret = c.sscanf(buf[2..].ptr, "%d;%d", &rowPos, &colPos);
    if (ret != 2) {
        return error.Termcap;
    }
...
fn getWindowSize() TerminalError![2]u16 {
    ...
    var ret2 = c.write(c.STDOUT_FILENO, "\x1b[999C\x1b[999B", 12);
    if (ret2 != 12) {
...
fn editorRowInsertChar(cfg: *Config, row: *buffer.Row, insert_at: usize, ch: u8) !void {
...
fn editorRowsToString(cfg: *Config) ![]u8 {
    ...
    for (cfg.rows.toSlice()) |row| {
        std.mem.copy(u8, result[idx_result .. idx_result + row.len()], row.chars[0..]);
        idx_result += row.len();
        result[idx_result] = '\n';
        idx_result += 1;
    }
...
fn editorSave(cfg: *Config) !void {
    ...
    if (std.io.writeFile(filename, buf)) |_| {
        cfg.dirty = 0;
        try editorSetStatusMessage(cfg, "{} bytes written to disk", buf.len);
    } else |err| {
        try editorSetStatusMessage(cfg, "Can't save! I/O error: {}", err);
    }
}

//// find ////

fn editorFindCallback(cfg: *Config, query: []u8, ch: u16) anyerror!void {
    ...
    while (idx < cfg.rows.count()) : (idx += 1) {
        ...
        if (row.find(query, 0)) |col| {
            ...
            std.mem.set(EditorHighlight, row.hl[col .. col + query.len], EditorHighlight.Match);
            break;
        }
    }
}
...
fn editorFind(cfg: *Config) !void {
    ...
    const query = try editorPrompt(cfg, "Search: {s} (Use ESC/Arrows/Enter)", editorFindCallback);
    ...
    while (y < cfg.screenRows) : (y += 1) {
        ...
        if (filerow >= cfg.numRows) {
            if (cfg.numRows == 0 and y == cfg.screenRows / 3) {
                ...
                var output = try std.fmt.bufPrint(welcome[0..], "Kilo editor -- version {}", KILO_VERSION);
            ...
            while (j < len) : (j += 1) {
                ...
                if (std.ascii.isCntrl(ch[0])) {
                    ...
                    if (current_color) |cc| {
                        ...
                        const clen = try std.fmt.bufPrint(buf[0..], "\x1b[{d}m", cc);
...
fn editorSetStatusMessage(cfg: *Config, comptime format: []const u8, args: anytype) !void {
    if (cfg.statusMsg) |sm| {
        allocator.free(sm);
    }
...
fn editorMoveCursor(cfg: *Config, key: u16) void {
    ...
    var row: ?*(buffer.Row) =
        if (cfg.cursorY >= cfg.rows.len) null else cfg.rows.at(cfg.cursorY);
    ...
    row = if (cfg.cursorY >= cfg.numRows) null else cfg.rows.at(cfg.cursorY);
    var rowlen = if (row == null) u16(0) else row.?.len();
...
fn editorProcessKeypress(cfg: *Config) !void {
    ...
    switch (key) {
        ...
        @enumToInt(EditorKey.PgUp), @enumToInt(EditorKey.PgDn) => {
            ...
            while (times > 0) : (times -= 1) {
                editorMoveCursor(cfg, if (key == @enumToInt(EditorKey.PgUp))
                    @enumToInt(EditorKey.ArrowUp)
                else
                    @enumToInt(EditorKey.ArrowDown));
            }
